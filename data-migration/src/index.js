...
async function migrateData() {
    ...
    try {
        ...
        // CSV 파일 읽기
        const csvFilePath = path.join(__dirname, '../data/notion-export.csv');
        const parser = fs
        ...
        for await (const record of parser) {
            ...
            const complaintData = {
                ...
                complainant_tel_no: record['신고자연락처'] || '',
                complainant_truck_no: record['차량번호'] || '',
                complainant_con_no: record['컨테이너번호'] || '',
                target_service: record['서비스명'] || '',
                target_terminal: record['터미널'] || '',
                complaint_status: record['처리상태'] || '',
                complaint_receiver: record['접수자'] || '',
                complaint_handler: record['처리자'] || '',
                complaint_type: record['민원유형'] || '',
                complaint_detail_type: record['상세민원유형'] || '',
                complaint_title: record['민원내용'] || '',
                complaint_content: record['문의상세'] || '',
                complaint_processing: record['처리내용'] || '',
                complaint_handling: record['민원처리'] || ''
            };
